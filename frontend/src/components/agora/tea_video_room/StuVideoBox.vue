<template>
  <div>
    <div class="livepart" id="live-box">
...
export default {
  ...
  mounted() {
    let _this = this
    this.getDevices()
    bus.$on('switchLiving', function(data) {
      _this.switchstu()
    })
    bus.$on('hangLiving', function(data) {
      _this.stop()
    })
  },
  methods: {
    ...
    switchstu: function() {
      SocketInstance.emit('sendOpenMsg', {})
      this.init()
      this.join()
    },
    stop: function() {
      SocketInstance.emit('sendCloseMsg', {})
    },
    init: function() {
      let _this = this
      _this.client.init(
        ...
        function(uid) {
          if (true) {
            ...
            _this.localStream.init(
              ...
              function(err) {
                _this.$meaage.error('getUserMedia failed ' + err)
              }
            )
          }
        },
    ...
    unpublish: function() {
      ...
      this.client.unpublish(_this.localStream, function(err) {
