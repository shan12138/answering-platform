  ...
  mounted() {
    ...
    SocketInstance.on('initOnlineList', function(data) {
      ...
      if (data.userid === _this.userid) {
        _this.$router.push('/student/livingRoomInfo')
      }
    })
  },
  methods: {
    ...
    initOnlineList() {
      ...
      SocketInstance.on('initOnlineList', function(data) {
        ...
        if (data.userid === _this.userid) {
          _this.$message('不好意思，同学，你已被老师移出房间')
          _this.$router.push('/student/home')
        }
      })
    },
    insertUser() {
    ...
    getStudentState() {
      ...
      this.$axios
        .post('/student/enter_room', this.$Qs.stringify(requestData))
        .then(response => {
          let responseData = response.data
          let { code, msg, data } = responseData
          if (code === '0000') {
            this.userState = data.authenticated_state
            this.userVerify = data.spoken_state
          } else {
