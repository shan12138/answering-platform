...
export default {
  ...
  computed: {
    searchKeyWord: function() {
      let _this = this
      let visibleTableData = _this.studentLogList.filter(data => {
        const studentName = data.name.toLowerCase()
        return (
          !_this.keyWord || studentName.includes(_this.keyWord.toLowerCase())
        )
      })
